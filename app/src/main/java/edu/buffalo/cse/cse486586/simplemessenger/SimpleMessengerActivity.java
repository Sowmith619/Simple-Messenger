            ...
            try {
                try {
                    while (true) {
                        ...
                        //we use flush as we are using println
                        //https://stackoverflow.com/questions/15479697/printwriter-or-any-other-output-stream-in-java-do-not-know-r-n/
                        PrintWriter server_ack = new PrintWriter(client.getOutputStream(), true);
                        //Using an acknowledge message to check for the socket to close at client end.
                        //we are sending the acknowldgement message from server.
                        String testmsg = null;
                        if ((testmsg = message.readLine()) != null) {
                            Log.i(TAG, "Received : " + testmsg);
                            server_ack.println(ACK_MSG);
                            Log.i(TAG, "Sent ACK : " + ACK_MSG);
                            publishProgress(testmsg);
                        }
                    }
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
                 ...
                 */
                PrintWriter Clientmsg = new PrintWriter(socket.getOutputStream(), true);
                BufferedReader ackclient = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                //checking whether we received the acknowledge message we received or not.
                //only if the acknowledgement message from the server we close the socket.
                Clientmsg.println(msgToSend);
                Log.i(TAG,"Client" +msgToSend);
                while(!socket.isClosed()){
                    String ack = ackclient.readLine();
                    if(ack.equals(ACK_MSG)){
